  ...
  return (
    <Card className="border border-border bg-card/90 shadow-sm">
      ...
      <CardContent className="px-4 pb-4">
        <div
          {...getRootProps()}
          className="flex min-h-[220px] flex-col items-center justify-center gap-4 rounded-3xl border border-dashed border-border bg-muted/30 p-8 text-center transition hover:border-primary"
        >
          <input {...getInputProps()} />
          <FileTextIcon className="size-10 text-muted-foreground" />
          <div>
            <p className="text-base font-medium text-foreground">Drop files here</p>
            <p className="text-sm text-muted-foreground">CSV, JSON, XLSX, or text extracts are supported.</p>
          </div>
